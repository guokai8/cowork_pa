...
Load the list of samples analyzed in Finn et al. 2009.
```{r}
...
```
There are `r ncol(hoeflich_2009)` subtypes common to both Hoeflich et al. 2009 and Finn et al. 2009, which we will use in our analysis to identify gene signatures resistant to CDK4/6 inhibitor (e.g. PD 0332991).
...
Before the groups are combined, the following table shows gene expression values of four samples at WFDC2 and MAPK1, for example.
```{r}
with(h2009_exprs, {
...
Transform the gene expression values (1 to `r as.integer(2^16 - 1)`) to a log scale (0 to 16).
```{r}
...
The list of samples analyzed in Finn et al. 2009 are:
```{r echo = FALSE}
